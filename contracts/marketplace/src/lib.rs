...
use stellai_lib::{
    ...
    types::{WorkflowStep, WorkflowStepStatus},
    validation,
};

// ── Execution-hub storage keys ────────────────────────────────────────────────
const HUB_KEY: &str = "exec_hub";
const PENDING_SALE_PREFIX: &str = "psale_";
const WF_LISTING_PREFIX: &str = "wf_lst_";

/// Pending sale state held while the execution-hub workflow drives the steps.
#[derive(Clone)]
...
impl MarketplaceContract {
    ...
    pub fn create_listing(
        ...
        let listing = Listing {
            listing_id,
            asset_id: agent_id,
            asset_type: stellai_lib::types::AssetType::Agent,
            seller: seller.clone(),
            price,
    ...
    pub fn buy_agent(env: Env, listing_id: u64, buyer: Address) {
        ...
        let _royalty_info = Self::get_royalty(env.clone(), listing.asset_id);
    ...
    fn execute_approved_listing_sale(env: Env, approval_id: u64, listing_id: u64) {
        ...
        let royalty_info = Self::get_royalty(env.clone(), listing.asset_id);
    ...
    // =========================================================================
    // Execution-Hub Integration (Issue #246)
    // =========================================================================

    /// Set the execution-hub contract address. Admin only.
    pub fn set_execution_hub(env: Env, admin: Address, hub: Address) {
        ...
        env.events()
            .publish((Symbol::new(&env, "hub_set"),), (hub,));
    }

    /// Get the configured execution-hub address.
    ...
    /// Purchase an agent by orchestrating a three-step workflow through the
    /// execution hub: verify → transfer_ownership → record_sale.
    ///
    /// Returns `(listing_id, workflow_id)`.
    pub fn buy_via_hub(env: Env, listing_id: u64, buyer: Address, amount: i128) -> (u64, u64) {
        buyer.require_auth();
        ...
        // Persist pending-sale record (workflow_id back-filled after hub call)
        ...
        let steps = Self::build_hub_sale_steps(&env, &marketplace, listing_id);
        ...
        env.events().publish(
            (Symbol::new(&env, "hub_sale_init"),),
            (listing_id, buyer, workflow_id, env.ledger().timestamp()),
        );

        // Drive step 0 immediately
    ...
    // ── Workflow step functions (invoked by the execution hub) ─────────────────

    /// Step 0 — verify listing and escrow are still valid.
    pub fn verify_sale(env: Env, encoded_args: Bytes) {
        let listing_id = Self::hub_decode_u64(&encoded_args);
        let listing_key = (Symbol::new(&env, "listing"), listing_id);
        let listing: Listing = env
            .storage()
            .instance()
        ...
        env.events().publish(
            (Symbol::new(&env, "hub_sale_vfy"),),
            (listing_id, env.ledger().timestamp()),
        );
    }

    /// Step 1 — transfer agent ownership to the buyer.
    pub fn transfer_ownership(env: Env, encoded_args: Bytes) {
        ...
        // Update the agent's owner in the agent-NFT registry.
        // The agent record lives in the agent-nft contract's storage but is
        // also mirrored in this contract's instance storage under the shared
        // AGENT_KEY_PREFIX convention used by the workspace.
        let agent_key = (
            Symbol::new(&env, stellai_lib::AGENT_KEY_PREFIX),
            listing.asset_id,
        );
        if let Some(mut agent): Option<stellai_lib::types::Agent> =
        ...
        env.events().publish(
            (Symbol::new(&env, "hub_own_xfer"),),
            (listing.asset_id, listing.seller, pending.buyer, env.ledger().timestamp()),
        );
    }

    /// Step 2 — deactivate listing and emit sale record.
    pub fn record_sale(env: Env, encoded_args: Bytes) {
        ...
        let royalty_key = (Symbol::new(&env, "royalty"), listing.asset_id);
        ...
        env.events().publish(
            (Symbol::new(&env, "hub_sale_done"),),
            (listing_id, listing.asset_id, pending.buyer, seller_amount, royalty_amount),
        );
    }

    /// Rollback — compensating action called by the hub on workflow failure.
    pub fn rollback(env: Env, encoded_args: Bytes) {
        ...
        let listing_key = (Symbol::new(&env, "listing"), listing_id);
        if let Some(listing): Option<Listing> = env.storage().instance().get(&listing_key) {
            let agent_key = (
                Symbol::new(&env, stellai_lib::AGENT_KEY_PREFIX),
                listing.asset_id,
            );
                ...
                env.storage().instance().get(&agent_key)
            {
                if agent.owner == pending.buyer {
                    ...
                    env.storage().instance().set(&agent_key, &agent);
                    env.events().publish(
                        (Symbol::new(&env, "hub_rb_own"),),
                        (listing.asset_id, pending.buyer.clone(), pending.seller.clone()),
                    );
                }
            }
        }
    ...
    /// Dispatch entry point called by the execution hub for each workflow step.
    ...
    pub fn wf_done(env: Env, workflow_id: u64, status: u32) {
        ...
        let listing_key = (Symbol::new(&env, "listing"), lid);

        match status {
            2 => {
                env.storage().instance().remove(&wlk);
                env.events().publish(
                    (Symbol::new(&env, "hub_cb_ok"),),
                    (workflow_id, lid, env.ledger().timestamp()),
                );
            }
            3 | 4 | 5 => {
                // Reactivate listing so it can be purchased again
                if let Some(mut listing): Option<Listing> =
                    env.storage().instance().get(&listing_key)
                {
                    if !listing.active {
                        listing.active = true;
    ...
    /// Query pending sale state for a listing.
    ...
    fn build_hub_sale_steps(env: &Env, marketplace: &Address, listing_id: u64) -> Vec<WorkflowStep> {
        let encoded = Self::hub_encode_u64(env, listing_id);
    ...
    fn hub_encode_u64(env: &Env, value: u64) -> Bytes {
        Bytes::from_array(env, &value.to_be_bytes())
    }

    fn hub_decode_u64(data: &Bytes) -> u64 {
        if data.len() < 8 {
